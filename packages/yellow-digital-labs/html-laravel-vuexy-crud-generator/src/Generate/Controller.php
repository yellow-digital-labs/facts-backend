...
use YellowDigitalLabs\HtmlLaravelVuexyCrudGenerator\Generate\Traits\FileManipulations;
...
class Controller extends ClassGenerator
{
  ...
  protected function buildClass()
  {
    return view('brackets/admin-generator::' . $this->view, [
      ...
      // index
      'columnsToQuery' => $this->readColumnsFromTable($this->tableName)
        ...
        ->pluck('name')
        ->toArray(),
      'columnsToSearchIn' => $this->readColumnsFromTable($this->tableName)
        ->filter(function ($column) {
          return ($column['type'] == 'json' ||
            $column['type'] == 'text' ||
            $column['type'] == 'string' ||
            $column['name'] == 'id') &&
            !($column['name'] == 'password' || $column['name'] == 'remember_token');
        })
        ->pluck('name')
        ->toArray(),
