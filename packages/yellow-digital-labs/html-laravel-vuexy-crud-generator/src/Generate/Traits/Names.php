<?php namespace YellowDigitalLabs\HtmlLaravelVuexyCrudGenerator\Generate\Traits;

use YellowDigitalLabs\HtmlLaravelVuexyCrudGenerator\Generate\Controller;
use YellowDigitalLabs\HtmlLaravelVuexyCrudGenerator\Generate\Model;
...
trait Names
{
  ...
  protected function initCommonNames(
    ...
    if (
      !Str::startsWith($this->modelFullName, $startsWith = trim($modelGenerator->rootNamespace(), '\\') . '\Models\\')
    ) {
    ...
    $this->modelViewsDirectory = Str::lower(
      Str::kebab(
        implode(
          '/',
          collect(explode('\\', $this->modelWithNamespaceFromDefault))
            ->map(function ($part) {
              return lcfirst($part);
            })
            ->toArray()
